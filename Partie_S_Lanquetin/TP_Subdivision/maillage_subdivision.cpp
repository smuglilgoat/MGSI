...
//-----------------------------------------------------------------------------
//
// m�thodes de subdivision sur des maillages quelconques
//
//-----------------------------------------------------------------------------
///////////////////////////////////////////////////////////////////////////////

///////////////////////////////////////////////////////////////////////////////
// �tape de subdivision - m�thode de Doo-Sabin
///////////////////////////////////////////////////////////////////////////////
...
void MaillageSubdivision::subdivision_doo_sabin()
{
	...
	////-----------------------------------------------------------------------
	//// cr�ation des nouveaux sommets
	//// pour chaque face, un sommet est cr�� pour chaque sommet de la face
	//// pour chaque arete externe, deux sommets suppl�mentaires sont cr��s
	//// pour chaque face avec n sommets, un tableau SF des indices des 
	////  nouveaux sommets est cr�� : 
	////  SF[0] SF[1] ... SF[n-1] donnera une nouvelle face avec n sommets
	//// pour chaque arete, les indices des quatre nouveaux sommets sont
	////  conserv�s dans le tableau SA :
	...
	numerotation_elements(); // (re)numeroter les elements � partir de 0
	
	k=0;
	for (PtrFace f1 = f_first; f1 != FACE_NONE; f1 = f1->suiv)
	{
		int n = f1->nb_sommets(); // sommets de la face f1
		Point3D Sf1_coord[n];     // coordonn�es de ces sommets
		...
		f1->marque((ULONG)SF);   // lier le tableau SF � la face f1
		...
		// calcul des coordonn�es des n nouveaux sommets
...
// cout << endl;
		...
		// lier chaque couple de nouveaux sommets � l'arete correspondante
	...
	// cr�er pour chaque arete externe, deux sommets suppl�mentaires
	for (PtrArete a1 = a_first; a1 != ARETE_NONE; a1 = a1->suiv)
	if (a1->estExterne)
	{
		int ia1 = a1->num;
		// coordonn�es des deux nouveaux sommets - sch�ma de Chaikin
...
// }
	...
	////-----------------------------------------------------------------------
	//// cr�ation des tableaux des nouvelles faces 
	...
	// cr�ation des nouvelles faces correspondant aux faces
	for (PtrFace f1 = f_first; f1 != FACE_NONE; f1 = f1->suiv)
	{
		ULONG *SF = (ULONG *)(f1->marque()); 
	...
	// cr�ation des nouvelles faces quadrangulaires correspondant aux aretes
	for (int i=0; i<nb_arete; i++)
	{
		for (int j=0; j<4; j++)
			T[kT++] = SA[i][j];
	...
	// cr�ation des nouvelles faces correspondant aux sommets
	for (PtrSommet s1=s_first; s1!=SOMMET_NONE; s1=s1->suiv)
	{
		lAretes las1 = s1->aretes(); // aretes incidentes � s1
		lAretes::iterator ilas1 = las1.begin();
		PtrArete a1;
		int n=0;
		if (s1->estExterne)
		{
			// un sommet suppl�mentaire pour la face correspondant
			// � un sommet s1 externe
			a1=*ilas1;
...
// }
	...
	// destruction des �l�ments existants
	supprimer_elements();
...
///////////////////////////////////////////////////////////////////////////////
// �tape de subdivision - m�thode de Catmull-Clark
///////////////////////////////////////////////////////////////////////////////
void MaillageSubdivision::subdivision_catmull_clark()
{
	...
	////-----------------------------------------------------------------------
	//// cr�ation des nouveaux sommets
	//// chaque nouveau sommet d'indice k est li� � l'�l�ment existant
	//// au moyen de sa marque
	ULONG k;
	
	// cr�ation d'un nouveau sommet pour chaque sommet 
	// indice de 0 � nb_sommet-1
	k = 0;
	for (PtrSommet s1 = s_first; s1 != SOMMET_NONE; s1 = s1->suiv)
		s1->marque(k++); 
		
	// cr�ation d'un nouveau sommet pour chaque arete
	// indice de nb_sommet � nb_sommet+nb_arete-1
	k = nb_sommet;
	for (PtrArete a1 = a_first; a1 != ARETE_NONE; a1 = a1->suiv)
		a1->marque(k++); 
		
	// cr�ation d'un nouveau sommet pour chaque face
	// indice de nb_sommet+nb_arete � nb_sommet+nb_arete+nb_face-1
	k = nb_sommet+nb_arete;
	for (PtrFace f1 = f_first; f1 != FACE_NONE; f1 = f1->suiv)
		f1->marque(k++); 
		
	////-----------------------------------------------------------------------
	//// cr�ation du tableau des coordonn�es des sommets 
	Point3D *Scoord;
	try {
		Scoord = new Point3D[nb_ns];
	}
	...
	// coordonn�es des nouveaux sommets correspondants aux faces existantes 	
	for (PtrFace f1 = f_first; f1 != FACE_NONE; f1 = f1->suiv)
	{
		k = f1->marque();
	...
	// coordonn�es des nouveaux sommets correspondants aux aretes existantes 	
	for (PtrArete a1 = a_first; a1 != ARETE_NONE; a1 = a1->suiv)
	{
		Point3D C;
	...
	// coordonn�es des nouveaux sommets correspondants aux sommets existants 	
	for (PtrSommet s1 = s_first; s1 != SOMMET_NONE; s1 = s1->suiv)
	{
		Point3D C;
		if (s1->estExterne)
...
//			C = (Q+2*R+S)/4;       // maillage quadrangle r�gulier		   
		}
		k = s1->marque(); 
		Scoord[k] = C;
	}
...
// }
	...
	////-----------------------------------------------------------------------
	//// cr�ation des tableaux des nouvelles faces (quadrangulaires)
	int tailleT = 4*nb_nf;
	ULONG *T;
	ULONG *TS;
	ULONG kT = 0;
	...
	for (PtrFace f1 = f_first; f1 != FACE_NONE; f1 = f1->suiv)
	{
		...
		// indice du nouveau sommet correspondant � f1
		...
		while (i1 != laf1.end())
		{
			...
			PtrArete a1 = *i1;
			PtrArete a2 = *i2;
			// indice du nouveau sommet correspondant � a1
			ULONG isa1 = a1->marque();
			// indice du nouveau sommet correspondant � a2
			ULONG isa2 = a2->marque();
			
			// sommet commun � a1 et a2
			PtrSommet s12;
			s12 = (a1->face1() == f1 ? a1->sommet2() : a1->sommet1());
			
			// indice du nouveau sommet correspondant � s12
			ULONG iss12 = s12->marque();
			
			// cr�ation d'une nouvelle face quadrangulaire
			TS[kTS+1] = TS[kTS]+4; kTS++;
			T[kT++] = isf1;
			T[kT++] = isa1;
			T[kT++] = iss12;
...
// - chaque face de degr� n donne une face de degr� n 
// - chaque sommet de degr� n>=3 donne une face de degr� n 
///////////////////////////////////////////////////////////////////////////////
void MaillageSubdivision::subdivision_midedge()
{
	...
	// calculer la somme des degr�s des nouvelles faces
	ULONG tailleT = 0;
	for (PtrFace f1 = f_first; f1 != FACE_NONE; f1 = f1->suiv)
	...
	// pour chaque face existante, cr�er une nouvelle face
	for (PtrFace f1 = f_first; f1 != FACE_NONE; f1 = f1->suiv)
	{
		lAretes laf1 = f1->aretes();
		
		// cr�er la face dont les sommets sont les milieux des aretes
	...
	// pour chaque sommet de degr� >= 3, cr�er une nouvelle face 
	for (PtrSommet s1 = s_first; s1 != SOMMET_NONE; s1 = s1->suiv)
	if (s1->nb_sommets() >= 3) 
	{
		lAretes las1 = s1->aretes();
...
///////////////////////////////////////////////////////////////////////////////
//
// routines sp�cifiques pour des maillages triangulaires
//
///////////////////////////////////////////////////////////////////////////////
///////////////////////////////////////////////////////////////////////////////

///////////////////////////////////////////////////////////////////////////////
// r�cuperer les 3 aretes de la face triangulaire f
///////////////////////////////////////////////////////////////////////////////
...
///////////////////////////////////////////////////////////////////////////////
// r�cuperer les 3 sommets de la face triangulaire f
///////////////////////////////////////////////////////////////////////////////
...
///////////////////////////////////////////////////////////////////////////////
// pour l'arete a1, trouver le sommet oppose par rapport � la face nf (1 ou 2)
///////////////////////////////////////////////////////////////////////////////
...
///////////////////////////////////////////////////////////////////////////////
// pour l'arete a1, trouver le sommet oppose par rapport � la face f1
///////////////////////////////////////////////////////////////////////////////
...
//-----------------------------------------------------------------------------
//
// m�thodes de subdivision sur des maillages triangulaires
//
//-----------------------------------------------------------------------------
///////////////////////////////////////////////////////////////////////////////
...
void MaillageSubdivision::subdivision_sqrt3()
{
	...
	// cr�ation d'un nouveau sommet pour chaque face
	// indice de nb_sommet � nb_sommet+nb_face-1
	k = nb_sommet;
	for (PtrFace f1 = f_first; f1 != FACE_NONE; f1 = f1->suiv)
		f1->marque(k++); 
		
	// cr�ation d'un nouveau sommet pour chaque arete externe 
	// indice de nb_sommet+nb_face � nb_sommet+nb_face+nb_s_externes()-1
	k = nb_sommet+nb_face;
	for (PtrArete a1 = a_first; a1 != ARETE_NONE; a1 = a1->suiv)
	if (a1->estExterne)
		a1->marque(k++); 
	...
	// coordonn�es des nouveaux sommets correspondants aux milieux des faces 	
	for (PtrFace f1 = f_first; f1 != FACE_NONE; f1 = f1->suiv)
	{
		k = f1->marque();
		Scoord[k] = f1->barycentre();
	...
	////-----------------------------------------------------------------------
	//// cr�ation des tableaux des nouvelles faces (toutes triangulaires)
	int tailleT = 3*nb_nf;
	ULONG *T;
	ULONG *TS;
	ULONG kT = 0;
	...
	// pour chaque arete, cr�er deux nouvelles faces 
	for (PtrArete a1 = a_first; a1 != ARETE_NONE; a1 = a1->suiv)
	{
		...
		if (a1->estExterne)
		{
			...
			if (f1 != FACE_NONE)
			{
				// face f1 existante : cr�er deux nouvelles faces
				// [is1,ia1,if1] et [ia1,is2,if1]			
				ULONG if1 = f1->marque();				

				// cr�ation d'une nouvelle face triangulaire
				T[kT++] = is1;
				T[kT++] = ia1;
				T[kT++] = if1;
			...
			{
				// face f2 existante : cr�er deux nouvelles faces
		...
		{
			// cr�er deux faces [is1,if2,if1] et [is2,if1,if2]
			ULONG if1 = f1->marque();				
			ULONG if2 = f2->marque();	
...
void MaillageSubdivision::subdivision_loop()
{
	...
	// pour chaque sommet, cr�er une nouvelle face pour chaque face incidente
	for (PtrSommet s1 = s_first; s1 != SOMMET_NONE; s1 = s1->suiv)
	{
		ULONG is1 = s1->marque(); 
		lAretes las1 = s1->aretes();
