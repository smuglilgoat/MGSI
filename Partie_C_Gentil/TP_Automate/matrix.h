...
public:
	...
	template <class > friend std::istream& operator>>(std::istream& s, matrix<t>& m);
...
// skips empty lines then reads vectors until meets an empty line
template <class t> std::istream& operator>>(std::istream& s, matrix<t>& m) {
////m.rows = 0;
////// remember current position; count number of rows then return the the initial position
////std::streampos current = s.tellg();
////char line[1024];
////while (!s.eof() && s.good()) {
////	s.getline(line, 1024);
////	if (0!=strcmp(line,"")) {
...
////s.seekg(current);
////s.clear();
////m.m = new vector<t>[m.rows];
////for (int i=0; i<m.rows; i++) {
////	s >> m.m[i];
////	//			std::cout << i << ": " << m.m[i];
////	assert(m.m[0].length()==m.m[i].length());
////}
////m.cols=m.m[0].length();
	return s;
}
